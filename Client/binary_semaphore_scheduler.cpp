...
BinarySemaphoreScheduler::BinarySemaphoreScheduler(QObject *parent) : QObject(parent) {
    ...
    connect(simulationTimer, &QTimer::timeout, this, &BinarySemaphoreScheduler::updateSimulation);
}

void BinarySemaphoreScheduler::setupGanttChart(QGraphicsView *view) {
    ganttView = view;
...
void BinarySemaphoreScheduler::setProcesses(const QList<Process>& newProcesses) {
...
void BinarySemaphoreScheduler::setResources(const QList<Resource>& newResources) {
    resources = newResources;
    initializeResources();
}

void BinarySemaphoreScheduler::setActions(const QList<Action>& newActions) {
    actions = newActions;
...
void BinarySemaphoreScheduler::assignProcessColors() {
    processColors.clear();
...
void BinarySemaphoreScheduler::initializeResources() {
    resourceSemaphores.clear();
    for (const Resource& r : resources) {
        semaphore sem;
        sem.value = (r.counter > 0) ? 1 : 0; // Binario: 1 o 0
        resourceSemaphores[r.name] = sem;
    }
}

void BinarySemaphoreScheduler::startSimulation() {
    currentTime = 0;
    hasCurrentProcess = false;
    currentProcessRemainingTime = 0;
    readyQueue.clear();
...
void BinarySemaphoreScheduler::stopSimulation() {
    simulationTimer->stop();
    calculateMetrics();
}

void BinarySemaphoreScheduler::updateSimulation() {
...
void BinarySemaphoreScheduler::processActions() {
...
void BinarySemaphoreScheduler::handleSemaphoreOperation(const Action& action) {
    ...
    if (action.operation == "READ" || action.operation == "WRITE") {
        ...
        QTimer::singleShot(2000, [this, action, process]() {
...
void BinarySemaphoreScheduler::executeCurrentProcess() {
...
void BinarySemaphoreScheduler::calculateMetrics() {
