...
FiFoScheduler::FiFoScheduler(QObject *parent) : QObject(parent) {
    ...
    connect(simulationTimer, &QTimer::timeout, this, &FiFoScheduler::updateSimulation);
}

void FiFoScheduler::setupGanttChart(QGraphicsView *view) {
    ganttView = view;
...
void FiFoScheduler::setProcesses(const QList<Process>& newProcesses) {
...
void FiFoScheduler::assignProcessColors() {
    processColors.clear();
...
void FiFoScheduler::stopSimulation() {
...
void FiFoScheduler::calculateMetrics() {
